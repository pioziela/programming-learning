input_numbers_to_sort = [1, 4, 1, 8, 9, 101, 2, -1, 3, 7, 5, 2, 13, 0, 8, 8, 8]


def counting_sort(numbers_to_sort):
    multiplier = 1
    whether_done = False
    while whether_done is False:
        max_element = 10 * multiplier
        min_element = -10 * multiplier
        elements_and_numbers_of_elements_count = {}
        counter_of_elements = 0
        for element in numbers_to_sort:
            if element < min_element or element > max_element:
                whether_done = False
    ...
    for i in range(min_element, max_element+1):
        ...
        print(elements_and_numbers_of_elements_count)
    output_sort_numbers = []
    counter_decrease = 0
